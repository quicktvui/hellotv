    ...
    <qt-view class="search_result" ref="search_result" :clipChildren="false" @childFocus="childFocus"
        ...
        <qt-tabs ref="tabRef" :hideOnSingleTab="true" :tabContentBlockFocusDirections="tabContentBlockFocusDirections"
            ...
            <template v-slot:tab-item>
                <!-- 自定义tab类型 -->
                <qt-view class="waterfall_nav_item" :type="2" :clipChildren="false" :focusable="true" eventFocus
                    autoWidth ref="waterfall_nav_item_text">
    ...
    setup(props, context) {
        ...
        const initTab = async () => {
            tabRef.value?.initTab({
                defaultIndex: 0, defaultFocusIndex: -1, itemList: [
                    {
                        type: 2,
                        text: '全部',
                        titleSize: 40
        ...
        const onTabPageLoadData = async (pageIndex: number, pageNo: number, useDiff: boolean) => {
            // 搜索数据
            let tabPage = await appApi.getSearchResultPageData(pageNo, 20, props.keyword, '')
