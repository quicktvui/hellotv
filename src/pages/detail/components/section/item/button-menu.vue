<template>
  <qt-view class='button-menu-root' :style='bmStyle'>
    <qt-list-view
      ...
      <qt-view class='menu-list-item' :style='bmliStyle' flexStyle='${style}' :type='1'
               :focusable='true' eventFocus eventClick>
        <qt-view duplicateParentState class='menu-list-item-img' flexStyle='${iconStyle}'>
          <img src='${focusIcon}' duplicateParentState showOnState='focused'
               flexStyle='${iconStyle}' :focusable='false' />
          <img src='${icon}' duplicateParentState :showOnState="['normal','selected']"
               flexStyle='${iconStyle}' :focusable='false' />
        </qt-view>
        <qt-text
...
let bmlitStyle = config.buttonMenuSize == 'default' ? { marginTop: 15 } : {
  marginLeft: 10,
  marginTop: -5
}
  ...
  listData = [
    {
      ...
      style: config.buttonMenuSize == 'default' ? { width: 140, height: 140 } : {
        width: 160,
        height: 70
      },
      iconStyle: config.buttonMenuSize == 'default' ? { width: 46, height: 46 } : {
        width: 30,
        height: 30
      }
    },
    {
      ...
      style: config.buttonMenuSize == 'default' ? { width: 200, height: 140 } : {
        width: 200,
        height: 70
      },
      iconStyle: config.buttonMenuSize == 'default' ? { width: 46, height: 46 } : {
