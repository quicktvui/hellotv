...
async function onPlayerPrepared() {
  ...
  if (!channel.programs) {
    channel.programs = await getLiveSourceChannelProgram(liveSourceEpg.replace('{name}', channel.name).replace('{date}', today), channel.name)
  }
...
// 获取直播源频道节目数据
function getLiveSourceChannelProgram(url: string, channel: string): Promise<Program[]> {
  // console.log('huan-getLiveSourceChannelProgram-url', url, channel)
  return Promise.resolve(mockProgram[channel])
  return new Promise((resolve, reject) => {
    fetch(url)
